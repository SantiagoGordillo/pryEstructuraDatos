...
namespace pryEstructuraDatos
{
    public partial class frmBaseDeDatos : Form
    {
        ...
        {
            string varSQL = "SELECT * FROM Libro where idIdioma = 2 AND idAutor in (SELECT * FROM Libro where idAutor = 5) ORDER BY 1 ASC";
            objBaseDatos.Listar(dgbMain, varSQL);
            dgbMain.AutoResizeColumns();
        }
        ...
        {
            string varSQL = "SELECT * FROM Libro where idIdioma = 2 AND idAutor not in (SELECT * FROM Libro where idAutor = 5) ORDER BY 1 ASC";
            objBaseDatos.Listar(dgbMain, varSQL);
            dgbMain.AutoResizeColumns();
        }
